<?php
       ...
       $query = "SELECT name,species FROM `Animal` WHERE exhibit='Mountainous'";
       if (!empty($sort)) {
...
?>
   <?php
      if(isset($_POST['submit'])) {
         ...
         $ntwk = mysqli_connect($hostname, $username, $password, $database);
         $user = $_SESSION['username'];
         $query2 = "INSERT INTO ExhibitVisit (visitor,exhibit,visitTime) VALUES ('$user','Mountainous','$currentTime')";
...
<html>
   <head>
      <title>Visitor Exhibit Details for Mountainous</title>
   ...
   <body>
      ...
      <div class="row">
            ...
            $query3 = "SELECT COUNT(name) AS Num_of_Animals FROM `Animal` WHERE exhibit='Mountainous'";
            $query4 = "SELECT * FROM Exhibit WHERE name = 'Mountainous'";
            $hostname = "academic-mysql.cc.gatech.edu"; /*This is your hostname */
            $username = "cs4400_group53"; /*The user id you use to log in phpmyadmin */
            $password ="Efhjn754"; /* the password for phpmyadmin */
            $database = "cs4400_group53"; /* the name of the database that you wish to fetch data from */
            ...
            <h4>Exhibit Name: Mountainous <br><?php while($row = $filterQuery4-> fetch_assoc()) {
...
function sort(type) {
    window.location = './ExhibitDetailMountainous.php?sort=' + type;
}
</script>
   </body>
</html>